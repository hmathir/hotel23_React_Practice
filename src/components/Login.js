...
const Login = () => {
    ...
    const { SignInUsingEmail, googleLogin } = useContext(AuthContext);
    ...
    const handleSignIn = (e) => {
        ...
        SignInUsingEmail(email, password).then(() => {
    ...
    const handleGoogleSignIn = () => {
        googleLogin(googleProvider).then(() => {
            navigate(from, { replace: true });
    ...
    return (
        <div className='h-screen flex justify-center items-center' style={{ backgroundImage: `url(${background})`, backgroundRepeat: "no-repeat", backgroundSize: "cover", backgroundBlendMode: "overlay", backgroundColor: 'rgba(0, 0, 0, 0.4)', }}>
            <div className="md:w-5/12 mx-auto max-w-md p-8 space-y-3 rounded-xl bg-white dark:bg-gray-900 dark:text-gray-100">
                ...
                <form onSubmit={handleSignIn} action="" className="space-y-6 ng-untouched ng-pristine ng-valid">
                    ...
                    <button className="block w-full p-3 text-center rounded-sm text-white dark:text-gray-900 bg-violet-400">Sign in</button>
                </form>
                ...
                <p className="text-xs text-center sm:px-6 dark:text-gray-400">Don't have an account?
                    <Link rel="noopener noreferrer" to='/register' className="dark:text-gray-100 text-red-600 font-bold"> Sign Up</Link>
                </p>
            </div>
        </div>
    );
